...
} from "@/features/profile/hooks/use-organizations";
import { type OrganizationFormValues, organizationSchema } from "@/features/profile/schemas";
import type { Organization } from "@/features/profile/types";
...
export function OrganizationsSection() {
  const { data: items, isLoading, isError } = useOrganizationList();
    ...
    <section className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <OrganizationDialogContent editing={editing} onDone={() => setOpen(false)} />
        </Dialog>
      </div>
      ...
      <div className="flex flex-col gap-2">
        {items?.map((item) => (
          <OrganizationCard key={item.id} item={item} onEdit={() => openEdit(item)} />
        ))}
      </div>
    </section>
  );
...
function OrganizationCard({ item, onEdit }: { item: Organization; onEdit: () => void }) {
  const deleteOrganization = useDeleteOrganization();

  const handleDelete = () => {
    if (!window.confirm(`Delete "${item.organization_name}"?`)) return;
    deleteOrganization.mutate(item.id, { onError: () => toast.error("Failed to delete.") });
  };
...
function OrganizationDialogContent({
  editing,
  onDone,
}: {
  ...
  const onSubmit = (values: OrganizationFormValues) => {
    ...
    const mutation = editing
      ? updateOrganization.mutateAsync({ id: editing.id, payload })
...
function toFormValues(item: Organization): OrganizationFormValues {
  return {
